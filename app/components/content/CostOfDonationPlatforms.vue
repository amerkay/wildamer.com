...
const DONATION_DISTRIBUTION = [
    { amount: 3, percentage: 0.1580 },    // £3: 15.80%
    { amount: 5, percentage: 0.4270 },    // £5: 42.70%
    { amount: 10, percentage: 0.3640 },   // £10: 36.40%
...
const PLATFORMS: Platform[] = [
    ...
    {
        name: 'JustGiving',
        feeRate: (amt) => amt <= 3 ? 0.333 : amt <= 5 ? 0.20 : 0.17,
        includesStripe: false,
...
function calculatePlatformCost(platform: Platform, goal: number) {
    ...
    const breakdown = donations.map(({ amount, count }) => {
        ...
        const stripeFeePerDonation = platform.includesStripe ? 0 : (totalPayment * 0.015 + 0.20)
...
const baseline = computed(() => {
    ...
    const breakdown = donations.map(({ amount, count }) => {
        ...
        const stripeFeePerDonation = totalPayment * 0.015 + 0.20
        const stripeFeeBucket = stripeFeePerDonation * count
...
const platformResults = computed(() => {
    const goal = fundraisingGoal.value ?? 10000
    return PLATFORMS
        .map(p => calculatePlatformCost(p, goal))
...
<template>
    <div class="not-prose w-full space-y-6 border-2 border-border px-4 py-6 sm:px-6 rounded-lg max-w-3xl mx-auto">
        <!-- Header -->
        <div class="space-y-2">
            <h2 class="text-3xl font-bold text-foreground">
                UK Fundraising Platform Fee Calculator
            </h2>
            <p class="text-muted-foreground">
                Compare platform costs for your fundraising campaign
            </p>
        </div>
        ...
        <div class="bg-card rounded-lg p-6 shadow-sm border">
            <div class="space-y-4">
                ...
                <Slider v-model="sliderIndex" :min="0" :max="SLIDER_VALUES.length - 1" :step="1" class="pt-2 mb-3" />

                <div class="flex justify-between text-center text-sm font-bold text-muted-foreground w-full">
            ...
            <div class="space-y-3 pt-4 mt-6 border-t border-border">
                <p class="text-sm font-medium text-muted-foreground">
                    Ask donors to contribute towards fundraising costs (applies to your own form only) </p>
                ...
                <TableBody>
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div class="text-left">
                                                ...
                                                <div class="text-xs text-muted-foreground">
                                                    {{ (baseline.contributionRate * 100).toFixed(1) }}% donor
                                                    contribution
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-3 space-y-2 text-xs">
                                            <div class="mb-3">
                                                <HelpTooltip :content="getTooltipText(baseline.totalDonations)">
                                                    <span class="font-semibold text-muted-foreground">Breakdown
                                                        by
                                                        donation amount:
                                                    </span>
                                                    <Icon name="lucide:circle-question-mark" size="15" />
                                            ...
                                            <div v-for="item in baseline.breakdown" :key="item.amount"
                                                class="flex justify-between items-center py-1 border-t border-border/50 first:border-t-0">
                                                <span class="text-muted-foreground">
                                                    {{ item.count }} × £{{ item.amount }}
                                                </span>
                                                <span
                                                    class="font-mono text-right text-emerald-600 dark:text-emerald-400">
                    ...
                    <template v-for="result in platformResults" :key="result.platform">
                        <TableRow class="hover:bg-muted/50">
                                    ...
                                    <AccordionItem value="breakdown" class="border-none">
                                        <AccordionTrigger class="hover:no-underline p-0">
                                            <div class="text-left">
                                                ...
                                                <div class="text-xs text-muted-foreground">{{ result.description }}
                                                </div>
                                            </div>
                                        </AccordionTrigger>
                                        <AccordionContent>
                                            <div class="mt-3 space-y-2 text-xs">
                                                ...
                                                <div v-for="item in result.breakdown" :key="item.amount"
                                                    class="flex justify-between items-center py-1 border-t border-border/50 first:border-t-0">
                                                    <span class="text-muted-foreground">
                                                        {{ item.count }} × £{{ item.amount }}
                                                    </span>
                            ...
                            <TableCell class="text-right text-muted-foreground">
                                <span v-if="!result.includesStripe">
                                    −{{ fmt(result.stripeFees) }}
                                </span>
        ...
        <div class="text-xs text-muted-foreground">
            <p>
                Calculations based on 14k real donations.
                Platform fees obtained from "donor cover fees" live forms.
                Stripe fees used 1.5% + £0.20.
            </p>
        </div>
    </div>
</template>
