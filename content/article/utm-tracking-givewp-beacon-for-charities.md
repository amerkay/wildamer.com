...
image: /imgs/article-utm-tracking-for-charities/utm-tracking-for-charities.png
imageDark: /imgs/article-utm-tracking-for-charities/utm-tracking-for-charities-dark.png
isFeatured: true
featuredOrder: 3
...
![UTM Parameters URL Diagram](/imgs/article-utm-tracking-for-charities/utm-parameter-url-diagram.png)

1. **utm_source** &ndash Tracks _where_ the traffic is coming from \
   **Common values:** `instagram`, `facebook`, `tiktok`, `youtube`, `linkedin`, `newsletter`, `google`, `meta`, `twitter` (or `x`), `reddit`, `sms`

2. **utm_medium** &ndash Tracks the _type_ of marketing channel \
...
3. **utm_campaign** &ndash Tracks the _specific campaign name_ \
...
![Google's Campaign URL Builder](/imgs/article-utm-tracking-for-charities/google-campaign-url-builder.png)
...
![Meta Build a URL Parameter](/imgs/article-utm-tracking-for-charities/meta-build-url-parameter.png)
...
2. From your WordPress settings, navigate to `Settings -> Beacon Multi-Currency Forms` and check the boxes for `Beacon JavaScript` and `UTM Parameter Tracking`. \
   ...
   !["Beacon Multi-Currency Forms" WordPress Plugin UTM configs](/imgs/article-utm-tracking-for-charities/wp-bmcf-utm-config-settings.png)
...
4. Add the **`utm_*` custom fields to your Beacon's Subscription and Payment** entity types.
   ![Add UTM custom fields to Beacon](/imgs/article-utm-tracking-for-charities/beacon-add-custom-fields.png)
5. This is important: **Each Beacon form must have the `utm_*` URL data fields configured**.
   ![Add utm_* parameters to your Beacon form](/imgs/article-utm-tracking-for-charities/beacon-form-configure-utm.png)
...
1. Follow the [GiveWP tutorial](https://givewp.com/documentation/add-ons/google-analytics/).
   ![GiveWP Google Analytics Settings](/imgs/article-utm-tracking-for-charities/givewp-settings-google-analytics.png)
2. Make sure your n8n Upsert Subscription and Upsert Payment nodes passes the UTM parameters. That's all covered in my [GiveWP to Beacon n8n integration tutorial](/project/n8n-givewp-beacon-crm-workflow-integration).
   ![n8n GiveWP UTM Parameters](/imgs/article-utm-tracking-for-charities/n8n-givewp-utm-parameters.png)
